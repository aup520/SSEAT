...
class Finetune:
    def __init__(
        ...
        self.model = ResNet18()
        self.model.load_state_dict(torch.load('/home/workspace/wcl/atcode/model/cr_clntrained.pt3'))
    ...
    def before_task(self, cur_iter, init_model=False, init_opt=True):
        ...
        self.scheduler = MultiStepLR(self.optimizer, milestones=[10], gamma=0.1)
    ...
    # def get_dataloader(self, batch_size, n_worker, train_list, test_list,cur_iter):
    def get_dataloader(self,cur_iter):
        # Loader
        train_loader = None
        test_loader = None
        # x=["cln","fgsm","pgd_1","pgd_2","pgdf"]
        #  x=["cln","FGSM","BIM","PGD","RFGSM","MIFGSM","NIFGSM","SINIFGSM","DIFGSM","VNIFGSM","VMIFGSM"]
        # x=["cln","FGSM","BIM","PGD","RFGSM","NIFGSM","SINIFGSM","DIFGSM","VMIFGSM"]
        # x=["cln","FGSM","BIM","PGD","RFGSM","VNIFGSM"]
        # x=["cln","FGSM","BIM","PGD","RFGSM","SINIFGSM"]
        # x=["cln","FGSM","BIM","PGD","MIFGSM","SINIFGSM"]
        # x=["cln","FGSM","PGD","SINIFGSM","DIFGSM","VNIFGSM"]
        # x=["cln","BIM","RFGSM","MIFGSM","NIFGSM","VMIFGSM"]
        # x=["cln","FGSM","PGD","MIFGSM","SINIFGSM","VMIFGSM"]
        # x=["cln","MIFGSM","PGD","FGSM","SINIFGSM","BIM"]
        # x=['cln', 'FGSM', 'BIM', 'PGD', 'RFGSM', 'NIFGSM']
        # x=["cln","SINIFGSM","DIFGSM","PGD","VNIFGSM","FGSM"]
        x=["cln","DIFGSM","RFGSM","PGD","SINIFGSM","FGSM"]
        
        print(x)
        # x1=["cln","FGSM","BIM","PGD","RFGSM","MIFGSM","NIFGSM","SINIFGSM","DIFGSM","VNIFGSM","VMIFGSM"]
        test_list=[]
        train_list=[]
        ...
        for batch_idx,(data, target) in enumerate(train_list):
            for i in range(data.size()[0]):
                alist.append(dict(klass=cls[target[i]],file_name=nu,data=data[i].cpu(),label=target[i].cpu(),task=cur_iter))
                nu=nu+1
        ...
        return train_loader,test_list,alist
    ...
    def train(self, cur_iter, n_epoch, batch_size, n_worker, n_passes=1):
        ...
        # train_loader, test_loader,alist = self.get_dataloader(
        #     batch_size, n_worker, train_list, test_list,cur_iter
        # )
        train_loader, test_loader,alist = self.get_dataloader(
            self,cur_iter
        )
    ...
    def evaluation_ext(self, test_list):
        ...
        test_loader = DataLoader(
            test_dataset, shuffle=False, batch_size=8, num_workers=2
        )
    ...
    def uncertainty_sampling(self, samples, num_class):
        ...
        for i in range(num_class):
            ...
            if len(cls_df) <= mem_per_cls:
                ...
                uncertain_samples = cls_df.sort_values(by="uncertainty")
                s=mem_per_cls//10
                mem=  uncertain_samples[uncertain_samples["task"]==0][::jump_idx][:s]       
                memlist= mem.to_dict(orient="records")
                ret =ret+memlist
                ret =ret+uncertain_samples[~uncertain_samples.index.isin(mem.index)][::jump_idx][:9*s].to_dict(orient="records")
